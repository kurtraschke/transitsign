...
if (typeof metrorail == 'undefined') { var metrorail = {}; }


metrorail.main = function(opt_data, opt_sb) {
...
metrorail.predictions = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  ...
  for (var predictionIndex31 = 0; predictionIndex31 < predictionListLen31; predictionIndex31++) {
    ...
    metrorail.timeClass({time: predictionData31.Min}, output);
    output.append('>');
    metrorail.time({time: predictionData31.Min}, output);
    output.append('</td></tr>');
  }
  if (!opt_sb) return output.toString();
};
...
metrorail.incidents = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  ...
  for (var incidentIndex50 = 0; incidentIndex50 < incidentListLen50; incidentIndex50++) {
    ...
    metrorail.lines({lines: incidentData50.LinesAffectedArr}, output);
...
metrorail.lines = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
...
metrorail.time = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
...
metrorail.timeClass = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
