...
$(document).ready(function(){
    ...
    onBefore = function(currSlideElement, nextSlideElement, options, forwardFlag) {
        ...
        currModule.onHide();
        $('#slidetitle').fadeOut(250, function() {$('#slidetitle').html(nextModule.title()).fadeIn(250);});
        nextModule.onShow();
    ...
    socket.on('connect', function () {
        ...
        $.each(modules, function(key, theModule) {
            var moduleDiv = $($('<div />').attr('id', theModule.name)).data('module', theModule);
            slidecontainer.append(moduleDiv);
            iconcontainer.append($('<img>').attr({'src': theModule.icon, 'id': iconID(theModule) }));
